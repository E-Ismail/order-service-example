...
from order_line orderlines0_
         left outer join
     product product1_ on orderlines0_.product_id = product1_.id
...
### adding @Fetch(FetchMode.SUBSELECT) to OrderHeader
...
from order_line orderlines0_
         left outer join
     product product1_ on orderlines0_.product_id = product1_.id
where orderlines0_.order_header_id in (select orderheade0_.id
                                       from order_header orderheade0_
                                       where orderheade0_.customer_id = ?);